...
// Takeaway.com sends order data via webhook
// We store it as an Order entity with order_source = "takeaway_com"
...
Deno.serve(async (req) => {
  ...
  // Takeaway.com order payload — can be nested
  const order = payload?.order || payload;

  // Find user by takeaway_store_id
  ...
  try {
    ...
    const matchedUser = users.find((u: any) => u.takeaway_store_id && u.takeaway_store_id === String(storeId));
    if (matchedUser) {
  ...
  const parsedItems = lineItems.map((item: any) => {
    const extras = (item?.options || item?.choices || item?.extras || []).map((opt: any) => ({
