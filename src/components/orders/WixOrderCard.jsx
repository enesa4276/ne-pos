...
import { Globe, MapPin, Phone, Mail, ExternalLink, CheckCircle, ChefHat, Truck, PackageCheck, X } from 'lucide-react';
import { formatCurrency } from '@/lib/i18n';
import moment from 'moment';

const WIX_STATUS_CONFIG = {
...
export default function WixOrderCard({ order, onStatusChange, loading }) {
  const cfg = WIX_STATUS_CONFIG[order.status] || WIX_STATUS_CONFIG['pending'];
      ...
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <span className="font-bold text-sm">
            {order.customer_name || 'Online Sipariş'}
          </span>
          <Badge className={`text-xs border ${cfg.color}`}>{cfg.label}</Badge>
          {order.payment_status_wix && (
            <Badge variant="outline" className="text-xs">
              {order.payment_status_wix === 'PAID' ? '✓ Ödendi' : '⚠ Ödenmedi'}
            </Badge>
          )}
        </div>
